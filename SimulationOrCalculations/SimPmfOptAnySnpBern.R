...
dfGeneratedBinVals <-
  foreach(
    ...
    binVals <- GenerateAnySnpOptParamBernoulli(pAlleleMinor, 
...
colnames(dfGeneratedBinVals) <- getOptBernName(length(pAlleleIndex))
...
DfCorreAlleleSummaryBernoulli <- DfCorreAlleleSummary
...
# saveRDS(dfGeneratedBinVals, "simPmfParamOptBern3SnpBinValFisher10k.rds")
# saveRDS(DfCorreAlleleSummary, "simPmfParamOptBern3SnpSumValFisher10k.rds")


# colnames(DfCorreAlleleSummaryLoad)
# Load in file. 
# DfCorreAlleleSummaryLoad <- readRDS("simPmfParamOptBern3SnpSumValFisher10k.rds")
